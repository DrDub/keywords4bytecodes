...
/**
 * <!-- globalinfo-start --> The implementation of Back-Propagation Multi-Label
 * Learning (BPMLL) learner. The learned model is stored in {&#64;link
 * NeuralNet} neural network. The models of the learner built by {&#64;link
 * BPMLLAlgorithm} from given training data set.
 * <p/>
 * <!-- globalinfo-end -->
 * 
...
public class BPMLL extends MultiLabelLearnerBase {
	 ...
	 *            the seed value for pseudo-random generator
	 */
	...
	/**
	 * Sets the topology of hidden layers for neural network. The length of
	 * passed array defines number of hidden layers. The value at particular
	 * index of array defines number of neurons in that layer. If
	 * <code>null</code> is specified, no hidden layers will be created. <br/>
	 * The network is created when learner is being built. The input and output
	 * layer is determined from input training data.
	 * 
	 * @param hiddenLayers
	 * @throws IllegalArgumentException
	 *             if any value in the array is less or equal to zero
	 */
			...
			for (int value : hiddenLayers) {
				if (value <= 0) {
					throw new IllegalArgumentException(
							"Invalid hidden layer topology definition. "
	...
	/**
	 * Gets an array defining topology of hidden layer of the underlying neural
	 * model.
	 * 
	 * @return The method returns a copy of the array.
	 */
	public int[] getHiddenLayers() {
		return hiddenLayersTopology == null ? hiddenLayersTopology : Arrays
				.copyOf(hiddenLayersTopology, hiddenLayersTopology.length);
	}
	...
	public void setLearningRate(double learningRate) {
		if (learningRate <= 0 || learningRate > 1) {
			throw new IllegalArgumentException(
					"The learning rate must be greater than 0 and no more than 1. "
	...
	/**
	 * Sets the regularization cost term for weights decay. Must be greater than
	 * 0 and no more than 1.<br/>
	...
	public void setWeightsDecayRegularization(double weightsDecayCost) {
		if (weightsDecayCost <= 0 || weightsDecayCost > 1) {
			throw new IllegalArgumentException(
					"The weights decay regularization cost "
	...
	public void setTrainingEpochs(int epochs) {
		if (epochs <= 0) {
			throw new IllegalArgumentException(
					"The number of training epochs must be greater than zero. "
	...
	/**
	 * Sets whether attributes of instances data (except label attributes)
	 * should be normalized prior to building the learner. Normalization is
	 * performed on numeric attributes to the range {-1,1}).<br/>
	...
	protected void buildInternal(final MultiLabelInstances instances)
			throws Exception {
		...
		if (this.getDebug())
			System.err.print("Preparing input data... ");
		...
		for (int epoch = 0; epoch < epochs; epoch++) {
			if (getDebug())
				System.err.println("Starting epoch " + epoch + " at "
						+ System.currentTimeMillis());
			Collections.shuffle(trainData, new Random(1));
			for (int index = 0; index < numInstances; index++) {
				DataPair trainPair = trainData.get(index);
				double result = learnAlg.learn(trainPair.getInput(),
						trainPair.getOutput(), learningRate);
	...
	public TechnicalInformation getTechnicalInformation() {
		TechnicalInformation technicalInfo = new TechnicalInformation(
				Type.ARTICLE);
		...
		technicalInfo.setValue(Field.JOURNAL,
	...
	private ThresholdFunction buildThresholdFunction(List<DataPair> trainData) {
		if (1 > 0)
			return new ThresholdFunction(0.5);

		int numExamples = trainData.size();
		float[][] idealLabels = new float[numExamples][numLabels];
	...
	private NeuralNet buildNeuralNetwork(int inputsDim) {
		...
		if (hiddenLayersTopology == null) {
			...
			System.arraycopy(hiddenLayersTopology, 0, networkTopology, 1,
					hiddenLayersTopology.length);
			networkTopology[networkTopology.length - 1] = numLabels;
		}

		NeuralNet aModel = new BasicNeuralNet(networkTopology, NET_BIAS,
				ActivationTANH.class, randomnessSeed == null ? null
	...
	/**
	 * Prepares {@link MultiLabelInstances} data for the learning algorithm. <br/>
	 * The data are checked for correct format, label attributes are converted
	 * to bipolar values. Finally {@link Instance} instances are converted to
	 * {@link DataPair} instances, which will be used for the algorithm.
	 */
	...
	private boolean checkAttributesFormatMadeChanges = false;

	/**
	 * Checks {@link Instances} data if attributes (all but the label
	 * attributes) are numeric or nominal. Nominal attributes are transformed to
	 * binary by use of {@link NominalToBinary} filter.
	 * 
	 * @param dataSet
	 *            instances data to be checked
	 * @param inputAttributes
	 *            input/feature attributes which format need to be checked
	 * @return data set if it passed checks; otherwise <code>null</code>
	 */
	private Instances checkAttributesFormat(Instances dataSet,
		...
		for (Attribute attribute : inputAttributes) {
			if (attribute.isNumeric() == false) {
				if (attribute.isNominal()) {
					nominalAttrRange.append((attribute.index() + 1)
							+ rangeDelimiter);
				} else {
					// fail check if any other attribute type than nominal or
					// numeric is used
					return null;
				}
			}
		...
		if (nominalAttrRange.length() > 0) {
			checkAttributesFormatMadeChanges = true;
			nominalAttrRange.deleteCharAt(nominalAttrRange
					.lastIndexOf(rangeDelimiter));
			try {
				nominalToBinaryFilter = new NominalToBinary();
				nominalToBinaryFilter.setAttributeIndices(nominalAttrRange
						.toString());
				...
				throw new WekaException(
						"Failed to apply NominalToBinary filter to the input instances data.",
						exception);
			}
		}
	...
	public MultiLabelOutput makePredictionInternal(Instance instance)
		...
		if (nominalToBinaryFilter != null) {
			try {
				...
				throw new InvalidDataException(
						"The input instance for prediction is invalid. "
								+ "Instance is not consistent with the data the model was built for.");
			}
		} else {
			inputInstance = DataUtils.createInstance(instance,
		...
		if (numAttributes < model.getNetInputSize()) {
			throw new InvalidDataException(
					"Input instance do not have enough attributes "
							+ "to be processed by the model. Instance is not consistent with the data the model was built for.");
		}

		// if instance has more attributes than model input, we assume that true
		// outputs
		...
		float[] inputPattern = new float[inputDim];
		int indexCounter = 0;
		for (int attrIndex = 0; attrIndex < numAttributes; attrIndex++) {
			if (labelsAreThere && someLabelIndices.contains(attrIndex)) {
				continue;
			}
			inputPattern[indexCounter] = (float) inputInstance.value(attrIndex);
			indexCounter++;
		}
		...
		MultiLabelOutput mlo = new MultiLabelOutput(labelPredictions,
				labelConfidences);
		return mlo;
	}
}